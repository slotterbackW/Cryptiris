defmodule CryptirisWeb.FollowController do
  ...
  def index(conn, _params) do
    codes = Currency.list_follows_by_user(conn.assigns[:user].id)
    render(conn, "codes.json", codes: codes)
  end

  def create(conn, %{"code" => code}) do
    with {:ok, %Follow{} = follow} <- Currency.create_follow(%{ "code" => code, "user_id" => conn.assigns[:user].id}) do
      conn
      |> put_status(:created)
  ...
  def delete(conn, %{"code" => code}) do
    follow = Currency.get_follow_by_code!(code, conn.assigns[:user].id)
